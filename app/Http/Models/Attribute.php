...
//    public $timestamps = false;
    protected $fillable = ['attr_name','attr_type','un_number','attr_option_values','updated_at','created_at','deleted_at'];
    ...
    public function getAll($arr){
        if(empty($arr['id'])){
            ...
            $data = $this
                ->get();
            $value = DB::table('goods_attr')
                ->where('goods_id','=',$arr['id'])
                ->get();
            foreach ($data as $key=>$val){
                foreach ($value as $key1=>$val1){
                    if($val->id == $val1->attr_id){
                        $data[$key]->val = $val1->attr_value;
                    }
                }
